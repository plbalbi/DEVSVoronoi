...
class Neighbourhood:
    def __init__(self, neighbours_list):
        if len(neighbours_list) == 0:
            raise Exception(
                'Cannot generate neighbourhood with an empty neighbours list')
...
class RulesetBuilder:
    ...
    def build(self, neighbourhood):
        ...
        for neighbour in neighbourhood:
            rule_set = rule_set + \
                self.rule_pattern.replace(
                    CURRENT_CELL_EXPRESSION_KEY, print_cell_position(neighbour))
...
class VoronoiModel(Model):
    ...
    def register_initial_expressions(self):
        ...
        self.expression_register['initial_cell_values_dimension'] = \
            self.initial_cell_values.get_printable_dimension()

        self.expression_register['initial_cell_values_path'] = \
            self.initial_cell_values.write_to_file()

        self.expression_register['neighbours'] = \
            self.neighbourhood.print_neighbours_list(self)

        self.expression_register['rule_set'] = \
            self.rule_builder.build(self.neighbourhood)
    ...
    def replace_expressions(self):
        ...
        for expression_found in expression_finder.finditer(non_replaced_source):
            replaceable_expression = expression_found.group(0)
            expression_key = expression_found.group(1)

            try:
                resolved_value = self.expression_register[expression_key]
            except KeyError:
                raise Exception(
                    'Expression with key "{0}" was not found in expression register.'.format(expression_key))

            self.source = self.source.replace(
                replaceable_expression, resolved_value)
